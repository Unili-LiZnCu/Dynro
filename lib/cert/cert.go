...
func GenerateCert() error {
	...
	file, err := os.Create("cacert/DYNRO.crt")
	if err != nil {
		return err
	}
	pfile , err := os.Create("cacert/DYNRO_crt.pem")
	if err != nil {
		return err
	}
	defer file.Close()
	pem.Encode(file, &block)
	pem.Encode(pfile, &block)
	block = pem.Block{
		Type:    "RSA PRIVATE KEY",
		Headers: nil,
		Bytes:   x509.MarshalPKCS1PrivateKey(key),
	}
	file, err = os.Create("cacert/DYNRO.key")
	pfile, err = os.Create("cacert/DYNRO_key.pem")
	if err != nil {
		return err
	}
	pem.Encode(file, &block)
